/* REB_MOBILE_PHASE1F_CUSTOMER_SERVICE_CORE_SYNC_20260917 */
(function(){
  'use strict';
  ...
  function token(){if(CORE)return CORE.liveToken();var t=(localStorage.getItem(TOKEN_KEY)||'').trim(),n=t.toLowerCase();return t&&n!=='null'&&n!=='undefined'&&!/^pending-/i.test(t)?t:'';}
  function authHeaders(extra){if(CORE)return CORE.authHeaders(extra||{});var h=extra||{};var t=token();if(t)h.Authorization='Bearer '+t;return h;}
  function userName(){if(CORE)return CORE.displayName();try{var d=JSON.parse(localStorage.getItem(USER_KEY)||'{}')||{},p=d.profile||{};return String(p.name||p.username||d.username||'Account');}catch(e){return 'Account';}}
  ...
  function badge(n){
    ...
    lastKnownUnread=n;
    if(ids.badge){
      ids.badge.textContent=n>99?'99+':String(n);
      ids.badge.classList.toggle('show',n>0);
      ids.badge.setAttribute('aria-hidden',n>0?'false':'true');
    }
    if(ids.launcher){
      var label=n>0?('Customer Service, '+n+' unread message'+(n===1?'':'s')):'Open ReachEmpireBot Customer Service';
      ids.launcher.setAttribute('aria-label',label);
      ids.launcher.setAttribute('title',label);
    }
    try{window.dispatchEvent(new CustomEvent('reb:customer-chat-unread',{detail:{unread:n}}));}catch(_e){}
  }
  ...
  function build(){
    ...
    function isMobileChatClient(){
      ...
      var w=Math.min(
        ...
        Number((document.documentElement&&document.documentElement.clientWidth)||99999),
        Number((window.screen&&window.screen.width)||99999)
      );
      try{
        return w<=900 || Number(navigator.maxTouchPoints||0)>0 || /Android|iPhone|iPad|iPod|Mobile/i.test(ua) || !!(window.matchMedia&&(
          window.matchMedia('(pointer:coarse)').matches || window.matchMedia('(hover:none)').matches || window.matchMedia('(max-width:900px)').matches
        ));
      }catch(_e){return w<=900 || /Android|iPhone|iPad|iPod|Mobile/i.test(ua);}
    }
    function dedicatedPage(){return document.body&&(
    ...
    function applyChatFloatingGeometry(){
      if(dedicatedPage()){
        ...
        shell.style.setProperty('min-width','100vw','important');
      ...
      if(!isMobileChatClient()){
        ['inset','left','top','right','bottom','width','max-width','min-width','height','max-height','min-height','border-radius','margin','padding','border','box-shadow'].forEach(function(k){shell.style.removeProperty(k);});
        return;
      }
      shell.style.setProperty('position','fixed','important');
    ...
    function safeReturnTarget(){
      try{
        var raw=new URLSearchParams(window.location.search).get('from')||'';
        if(!raw || raw.charAt(0)!=='/' || raw.indexOf('//')===0 || /customer-service\.html/i.test(raw))return '';
        return raw;
      }catch(_e){return '';}
    }
    function closeChat(){
      if(dedicatedPage()){
        var target=safeReturnTarget();
    ...
    launcher.addEventListener('click',function(ev){
      if(dedicatedPage()){applyChatFloatingGeometry();toggle(true);return;}
      if(isMobileChatClient()){
        if(ev){ev.preventDefault();ev.stopPropagation();if(ev.stopImmediatePropagation)ev.stopImmediatePropagation();}
        var from=window.location.pathname+window.location.search+window.location.hash;
        window.location.href='/customer-service.html?v=20260917-phase1e&from='+encodeURIComponent(from);
        return;
      }
      applyChatFloatingGeometry();toggle(true);
    });
    qs('.reb-chat-close',shell).addEventListener('click',closeChat);
