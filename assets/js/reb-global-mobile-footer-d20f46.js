...
function boot(){
 ...
 nav.innerHTML=html();
 var k=key();nav.querySelectorAll('[data-reb-nav]').forEach(function(a){a.classList.remove('active');a.removeAttribute('aria-current');});var current=nav.querySelector('[data-reb-nav="'+k+'"]');if(current){current.classList.add('active');current.setAttribute('aria-current','page');}
 document.body.classList.add('reb-global-mobile-footer-enabled');
 var img=nav.querySelector('.reb-footer-profile-photo'),ico=nav.querySelector('.reb-footer-profile-fallback');
 function fallbackProfile(){if(img){img.hidden=true;img.removeAttribute('src');}if(ico)ico.style.display='';}
 fallbackProfile();
 if(img)img.onerror=fallbackProfile;
 var core=window.REBClientCore;
 var live=!!(core&&core.hasLiveSession&&core.hasLiveSession());
 if(live&&core.json){core.json('/api/social/profile/me',{method:'GET',timeoutMs:6000}).then(function(d){var p=d&&(d.profile||d.user||d);var u=String(p&&(p.avatar||p.avatar_url||p.profile_photo||p.profile_photo_url)||'').trim();if(!u){fallbackProfile();return;}if(img){img.onload=function(){img.hidden=false;if(ico)ico.style.display='none';};img.src=u;}}).catch(fallbackProfile);}else{fallbackProfile();}
 }
