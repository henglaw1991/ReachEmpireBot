/* ReachEmpire Social Center — V88 Desktop feed root-cause fix.
   Desktop: newest-first + deterministic initial scroll + priority feed request + shared profile hydration.
   Mobile behavior remains on the existing legacy branch. */
(function(){
  'use strict';
  ...
  var rebV88SocialRoot=(function(){
    if(!rebV82DesktopFast)return false;
    var p=String(window.location.pathname||'/').replace(/\/+$/,'')||'/';
    if(p!=='/social')return false;
    try{var u=new URLSearchParams(window.location.search||'');if(u.get('post')||u.get('profile'))return false;}catch(_e){}
    return !window.location.hash;
  })();
  if(rebV88SocialRoot){try{if('scrollRestoration' in history)history.scrollRestoration='manual';}catch(_e){}}
  var rebV88InitialScrollDone=false;
  function rebV88TopNow(){
    if(!rebV88SocialRoot)return;
    try{var a=document.activeElement;if(a&&/^(INPUT|TEXTAREA|SELECT)$/.test(a.tagName))a.blur();}catch(_e){}
    try{window.scrollTo(0,0);}catch(_e){}
    try{document.documentElement.scrollTop=0;if(document.body)document.body.scrollTop=0;}catch(_e){}
  }
  function rebV88LockInitialTop(){
    if(!rebV88SocialRoot||rebV88InitialScrollDone)return;
    rebV88InitialScrollDone=true;
    rebV88TopNow();
    window.requestAnimationFrame(function(){rebV88TopNow();window.requestAnimationFrame(rebV88TopNow);});
    window.setTimeout(rebV88TopNow,80);
    window.setTimeout(rebV88TopNow,220);
  }
  ...
  function feedCacheKey(){
    ...
    return 'REB_SOCIAL_FEED_CACHE_V88_'+(communityMode?'community':'feed')+'_'+currentFeed+'_'+user;
  }
  function readFeedCache(){
    try{
      var key=feedCacheKey(),raw=sessionStorage.getItem(key)||localStorage.getItem(key);if(!raw)return null;
      var d=JSON.parse(raw);if(!d||!d.saved||Date.now()-Number(d.saved)>(rebV82DesktopFast?900000:86400000)||!Array.isArray(d.posts))return null;
      return d;
  ...
  function scheduleSecondaryLoads(){
    /* Feed is the only critical request at first paint. Badge requests can wait. */
    var run=function(){loadUnread();loadMessageUnread();};
    if(rebV82DesktopFast)window.setTimeout(run,2400);
    else if(window.requestIdleCallback)window.requestIdleCallback(run,{timeout:1200});
  ...
  function isDesktopIdentity(){return !(window.matchMedia&&window.matchMedia('(max-width:767px)').matches);}
  function isVerifiedContext(person,source){
    if(isVerified(person))return true;
    if(!source)return false;
    var vals=[source.author_verified,source.author_is_verified,source.is_author_verified,source.author_blue_verified,source.author_blue_tick,source.verified_author,source.author_verification_status];
    return vals.some(function(v){return v===true||v===1||v==='1'||String(v||'').toLowerCase()==='true'||String(v||'').toLowerCase()==='verified'||String(v||'').toLowerCase()==='active';});
  }
  function makeVerifiedTick(){
    var vt=document.createElement('span');vt.className='reb-blue-verified';vt.title='Verified account';vt.setAttribute('aria-label','Verified account');
    vt.innerHTML='<svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><g fill="#1d9bf0"><circle cx="12" cy="5" r="4.6"/><circle cx="17" cy="7" r="4.6"/><circle cx="19" cy="12" r="4.6"/><circle cx="17" cy="17" r="4.6"/><circle cx="12" cy="19" r="4.6"/><circle cx="7" cy="17" r="4.6"/><circle cx="5" cy="12" r="4.6"/><circle cx="7" cy="7" r="4.6"/><circle cx="12" cy="12" r="6.5"/></g><path d="M7.5 12.2l3 3.05 6.25-6.5" fill="none" stroke="#fff" stroke-width="2.35" stroke-linecap="round" stroke-linejoin="round"/></svg>';
    return vt;
  }
  ...
  function renderSharedPostCard(shared){
    ...
    var sharedNameLine=document.createElement('div');sharedNameLine.className='reb-social-shared-name-line';sharedNameLine.appendChild(strong);if(isVerifiedContext(author,shared))sharedNameLine.appendChild(makeVerifiedTick());copy.appendChild(sharedNameLine);
    copy.appendChild(time);head.appendChild(av);head.appendChild(copy);box.appendChild(head);
  ...
  async function loadFeedDesktopFast(reset){
    ...
    try{
      ...
      feedLoading=false;els.loadMore.disabled=false;els.loadMore.textContent='Load more';
      if(reset!==false&&rebV88SocialRoot)rebV88LockInitialTop();
    }
  }
  ...
  function communityMemberRow(profile){
    ...
    var copy=document.createElement('button');copy.type='button';copy.className='reb-social-community-copy';var strong=document.createElement('strong');strong.textContent=String(profile.display_name||profile.username||'Member');var user=document.createElement('span');user.textContent='@'+String(profile.username||'');var metrics=document.createElement('small');metrics.textContent=String(Number(profile.followers_count||0))+' followers · '+String(Number(profile.posts_count||0))+' posts';var memberNameLine=document.createElement('span');memberNameLine.className='reb-social-community-name-line';memberNameLine.appendChild(strong);if(isVerified(profile))memberNameLine.appendChild(makeVerifiedTick());copy.appendChild(memberNameLine);copy.appendChild(user);copy.appendChild(metrics);row.appendChild(av);row.appendChild(copy);
  ...
  function wire(){
    ...
    if(rebV82DesktopFast){
      rebV88TopNow();
      renderAccount();
      loadFeed(true);
      handleDeepLink();
      scheduleSecondaryLoads();
      window.addEventListener('reb:social-profile-ready',function(event){var p=event&&event.detail&&event.detail.profile;if(p){me=p;renderAccount();}},{once:false});
    }else{renderAccount();loadMe().then(function(){loadFeed(true);loadUnread();loadMessageUnread();handleDeepLink();});}
  ...
  if(rebV88SocialRoot)window.addEventListener('pageshow',function(){rebV88TopNow();window.setTimeout(rebV88TopNow,60);},{once:true});
