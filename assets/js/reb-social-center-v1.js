/* ReachEmpire Social Center — Phase 3D: Profile & Community + stable Phase 3C media/share */
(function(){
  'use strict';
  ...
  function feedCacheKey(){
    ...
    return 'REB_SOCIAL_FEED_CACHE_V82_'+(communityMode?'community':'feed')+'_'+currentFeed+'_'+user;
  }
  ...
  function renderSharedPostCard(shared){
    ...
    if(isDesktopIdentity()){var sharedNameLine=document.createElement('div');sharedNameLine.className='reb-social-shared-name-line';sharedNameLine.appendChild(strong);if(isVerifiedContext(author,shared))sharedNameLine.appendChild(makeVerifiedTick());copy.appendChild(sharedNameLine);}else{copy.appendChild(strong);}
  ...
  async function loadFeedDesktopFast(reset){
    ...
    if(reset!==false){
      ...
      if(cached&&cached.posts.length){
        els.feed.innerHTML='';cached.posts.forEach(function(post){els.feed.appendChild(renderPost(post));});
    ...
    var path=communityMode?'/api/social/community/feed?limit=6':('/api/social/feed?mode='+encodeURIComponent(currentFeed)+'&limit=6');
  ...
  function communityMemberRow(profile){
    ...
    var copy=document.createElement('button');copy.type='button';copy.className='reb-social-community-copy';var strong=document.createElement('strong');strong.textContent=String(profile.display_name||profile.username||'Member');var user=document.createElement('span');user.textContent='@'+String(profile.username||'');var metrics=document.createElement('small');metrics.textContent=String(Number(profile.followers_count||0))+' followers · '+String(Number(profile.posts_count||0))+' posts';if(isDesktopIdentity()){var memberNameLine=document.createElement('span');memberNameLine.className='reb-social-community-name-line';memberNameLine.appendChild(strong);if(isVerified(profile))memberNameLine.appendChild(makeVerifiedTick());copy.appendChild(memberNameLine);}else{copy.appendChild(strong);}copy.appendChild(user);copy.appendChild(metrics);row.appendChild(av);row.appendChild(copy);
